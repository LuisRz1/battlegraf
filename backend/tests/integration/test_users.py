...
def test_list_school_users_as_director(
    test_client: TestClient, director, sample_school, auth_headers
):
    response = test_client.get(
        f"/api/v1/users/school/{sample_school.id}", headers=auth_headers
    )
...
def test_list_section_users_as_professor(
    test_client, professor, sample_section, professor_headers
):
    response = test_client.get(
...
def test_create_user_rejected_for_professor(
    test_client, sample_school, professor_headers
):
    payload = {
    ...
    response = test_client.post(
        "/api/v1/users", json=payload, headers=professor_headers
    )
...
def test_create_staff_professor(test_client, sample_school, auth_headers):
    payload = {
        ...
        "email": "professor_staff@example.com",
        "full_name": "Professor Staff",
        "password": "password123",
        "role": "professor",
    }
    response = test_client.post(
        "/api/v1/users/staff", json=payload, headers=auth_headers
    )
    assert response.status_code == 201
    data = response.json()
...
def test_create_staff_invalid_role(test_client, sample_school, auth_headers):
    payload = {
        ...
        "email": "director_staff@example.com",
...
def test_bulk_create_students_csv(
    test_client, sample_school, sample_section, auth_headers
):
...
def test_bulk_create_students_csv_existing_username(
    test_client, sample_school, sample_section, auth_headers
):
    csv_content = "username,full_name,password\nstudent1,Student One,pass123"
...
def test_bulk_create_students_csv_wrong_section(
    test_client, sample_school, auth_headers
):
    import uuid
...
def test_bulk_create_students_csv_invalid_headers(
    test_client, sample_school, sample_section, auth_headers
):
...
async def test_inactive_user_cannot_login(
    ...
    await user_repo.update(director)
    await db_session.commit()

    response = test_client.post(
        "/api/v1/auth/login",
        data={"username": director.username, "password": "password123"},
    )

    assert response.status_code == 403
    assert response.json()["detail"] == "La cuenta se encuentra desactivada"
