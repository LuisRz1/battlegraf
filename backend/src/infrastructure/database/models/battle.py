...
class GraphNodeModel(Base, UUIDMixin):
    ...
    connected_to: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False)
...
class BattleModel(Base, UUIDMixin):
    ...
    player_positions: Mapped[dict[str, str]] = mapped_column(
        JSON, default=dict, nullable=False
    )
    active_node_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid(as_uuid=True), nullable=True
    )
    active_question_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid(as_uuid=True), nullable=True
    )
    ...
    is_section_battle: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )

    node_states: Mapped[list["BattleNodeStateModel"]] = relationship(
...
class BattleNodeStateModel(Base):
    ...
    battle_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("battles.id"), primary_key=True
    )
    ...
    battle: Mapped[BattleModel] = relationship(
        "BattleModel", back_populates="node_states"
    )
...
class BattleMoveModel(Base, UUIDMixin):
    ...
    battle_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("battles.id"), nullable=False
    )
    ...
    is_steal_attempt: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )
