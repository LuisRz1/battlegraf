...
test("server-renders the separate institutional console", async () => {
  ...
  assert.match(html, /Panel institucional \| BattleGraph/i);
...
test("admin console includes medieval assets, working modals and simulated AI output", async () => {
  ...
  assert.match(css, /\/admin-assets\/button_frame_normal\.png/);
  assert.match(css, /\/admin-assets\/button_frame_hover\.png/);
  assert.match(css, /\/admin-assets\/button_frame_pressed\.png/);
  assert.match(css, /\/admin-assets\/button_frame_disabled\.png/);
...
test("server-renders the complete BattleGraph landing", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>BattleGraph \| Aprender es conquistar<\/title>/i);
...
test("uses CSS-built castles, hexagonal nodes and accessible game UI", async () => {
  ...
  assert.match(layout, /BattleGraph \| Aprender es conquistar/);
